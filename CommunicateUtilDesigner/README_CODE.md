...
本文档面向维护和二次开发 `CommunicateUtilDesigner` 的代码工程师，重点说明项目结构、职责边界和生成规则。
...
CommunicateUtilDesigner/
  CommunicateUtilDesigner.sln             # 设计器解决方案入口
  README.md                               # 设计器总览和文档入口
  README_CODE.md                          # 面向代码工程师的维护说明
  README_USER.md                          # 面向使用者的页面操作说明
  src/                                    # 设计器源码目录
    CommunicateUtil.Generator/            # 生成器核心类库，把协议配置转换为 C# 源码
      Models/                             # 生成请求、生成结果和协议配置模型
      CommunicationCodeGenerator.cs       # 代码生成主逻辑
      CommunicationSchemaValidator.cs     # 协议配置校验主逻辑
      ICommunicationCodeGenerator.cs      # 代码生成器接口
      ICommunicationSchemaValidator.cs    # 配置校验器接口
    CommunicateUtil.Web/                  # ASP.NET Core MVC Web 设计器
      Controllers/                        # 页面请求入口和表单提交处理
      Data/                               # SqlSugar 实体和数据库初始化
      Services/                           # 仓储、路径和业务组装服务
      Views/                              # Razor 页面模板
      wwwroot/                            # CSS、JS 和第三方静态资源
  tests/                                  # 自动化测试目录
    CommunicateUtil.Generator.Tests/      # 生成器 xUnit 测试
```
...
- 支持完整项目生成和局部源码生成。
- 生成结果默认输出到 `Models/`、`Enums/`、`Validation/`、`Examples/` 等目录，便于直接作为类库工程使用。
...
- 完整源码预览、下载 ZIP、本地写入。
- 当前类源码预览、当前枚举源码预览。
- 项目详情页把“项目、类、字段、枚举、校验方法”整合在同一工作台，减少页面切换成本。
